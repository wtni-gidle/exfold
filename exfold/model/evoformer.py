...
from exfold.model.dropout import DropoutRowwise
from exfold.model.msa import (
    MSARowAttentionWithPairBias,
    MSAColumnAttention,
    MSATransition,
)
from exfold.model.outer_product_mean import OuterProductMean
from exfold.model.pair_transition import PairTransition
from exfold.model.triangular_attention import TriangleAttention
from exfold.model.triangular_multiplicative_update import (
    TriangleMultiplicationOutgoing,
    TriangleMultiplicationIncoming,
    FusedTriangleMultiplicationIncoming,
    FusedTriangleMultiplicationOutgoing,
)
from exfold.utils.checkpointing import checkpoint_blocks
from exfold.utils.chunk_utils import ChunkSizeTuner
from exfold.utils.tensor_utils import add
...
class PairStack(nn.Module):
    ...
    def forward(
        ...
        Args:
            z:
                [*, N_res, N_res, C_m] Pair embedding
            pair_mask:
        ...
        # _add_with_inplace=True表示是否在inplace操作时直接进行加法操作
        # 另外_add_with_inplace没有用dropout, 因为inplace用于推理, 推理时dropout等价于identity
        if not inplace_safe:
        ...
        # endregion
        # region: tri_att_end
        # 用的是同一种TriangleAttention, 所以要做transpose
        z = z.transpose(-2, -3)
        if inplace_safe:
            z = z.contiguous()
...
class EvoformerBlock(MSAPairBlock):
    ...
    def forward(
        ...
        # 如果inplace为True, 则input_tensors中的tensor在操作过程中会被自动更新
...
class EvoformerStack(nn.Module):
    ...
    def _prep_blocks(
        ...
        """
        对每一个block设置共同的参数, clear cache, 设置chunk_size
        """
        blocks = [
            partial(
                b,
    ...
    def _forward_offload(
        ...
        """
        用于cpu_offloading的forward, inplace_safe自动为True
        """
