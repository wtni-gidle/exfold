...
class ExFold(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.recycling_embedder = RecyclingEmbedder(
            **self.config["recycling_embedder"],
            with_x=not self.globals.is_geom
        )

        self.evoformer = EvoformerStack(
            **self.config["evoformer_stack"],
            output_s=not self.globals.is_geom
        )
        
        if not self.globals.is_geom:
    ...
    def iteration(
        ...
        if m_1_prev is None: # x_prev=None is allowed if is_geom=True
            # [*, N_res, C_m]
            ...
            # For is_geom=True, we do not use x_prev for recycle.
            if not self.globals.is_geom:
        ...
        outputs["msa"] = m
        outputs["pair"] = z
        #todo if raising error (DDP) during training, check this
        outputs["single"] = s # if output_s=False, s=None
        # if self.globals.is_e2e:
